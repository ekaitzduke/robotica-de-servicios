...
def testgame(displaypath,savepath,imgformat,cameraport,saveframes,gathersamples,images,usefinger,debug,nomediapipe,recognizer):
    ...
    buttonPanel = ScreenDisplayPanel(BUTTONINITIALPOS,DISPLAYINITIALDIM,BUTTONOFFSET,BUTTONDISTRIB,[],text=BUTTONTEXT)
    setButtonImage(buttonPanel,BUTCOLORSTATEINI)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            # All mouse events
                ...
                if camara.dim[0] == CAMARAZOOMEDDIM[0] and camara.dim[1] == CAMARAZOOMEDDIM[1]:
                    if camara.pressed(pos):
                    ...
                    elif camaraPanel.updatepressed(pos):
                        for j in range(len(camaraPanel.states)):
                            if camaraPanel.states[j] > 0:
                                id = j
                        if pygame.mouse.get_pressed()[0]:
                            rightmouseim = id
                            camaraPanel.updatestates(leftmouseim,3,erase=False)
                        else:
                            leftmouseim = id
                            camaraPanel.updatestates(leftmouseim,3,erase=False)
                            camaraPanel.updatestates(rightmouseim,1,erase=False)

                        if rightmouseim > 0 and leftmouseim > 0 and buttonPanel.states[2] < 1:
                            setButtonImage(buttonPanel,BUTCOLORSTATESWAPREADY)
                        elif buttonPanel.states[1] < 1 or (buttonPanel.states[2] > 0 and rightmouseim == leftmouseim):
                            setButtonImage(buttonPanel,BUTCOLORSTATEIMAGESEL)


                    elif (len(buttonPanel.states) > 3) and buttonPanel.updatepressed(pos,-1,erase=False):
                        if buttonPanel.states[1] < 0:
                            buttonPanel.states[1] = 1
                        ...
                        elif buttonPanel.states[2] < 0:
                            buttonPanel.states[2] = 1
                            if sum(camaraPanel.states) > 3:
                                camaraPanel.text[leftmouseim][0], camaraPanel.text[rightmouseim][0] = camaraPanel.text[rightmouseim][0], camaraPanel.text[leftmouseim][0]
                                tempstorage = camaraPanel.storage[rightmouseim]
                                camaraPanel.update(camaraPanel.storage[leftmouseim],SCREENSIZE,rightmouseim)
                                camaraPanel.update(tempstorage,SCREENSIZE,leftmouseim)
                ...
                # Camera on normal mode (Ready to pass to zoomed mode once live camera pressed)
                    ...
                    if camara.pressed(pos):
                        ...
                        setButtonImage(buttonPanel,BUTCOLORSTATEZOOM)
                        manualControl = False

                    elif buttonPanel.states[0] > 0 and buttonPanel.updatepressed(pos,-1,erase=False):
                        if buttonPanel.states[0] < 0:
                            buttonPanel.states[0] = 2
                        elif buttonPanel.states[3] < 0:
                            manualControl = not manualControl
                            butcolorid = copy(BUTCOLORSTATEINI)
                            if manualControl:
                                butcolorid[3] = 3
                        
                            butcolorid[0] = buttonPanel.states[0]
                            setButtonImage(buttonPanel,butcolorid)


                    elif buttonPanel.updatepressed(pos,-1,erase=False) and buttonPanel.states[3] < 0:
                        manualControl = not manualControl
                        butcolorid = copy(BUTCOLORSTATEINI)
                        if manualControl:
                            butcolorid[3] = 3
                        ...
                        camaraPanel.updatepressed(pos,2)
            ...
            # All keybinds events
                ...
                elif key[ACKNOWLEDGEKEY] and buttonPanel.states[0] > 0:
                    buttonPanel.states[0] = 2
        ...
        # Enact the detection once timer runs out (and not on debug mode)
            ...
            if not manualControl and not nomediapipe:
                if usefinger:
            ...
            if buttonPanel.states[0] < 1:
                colorid = BUTCOLORSTATEACKNOWLEDGE
                if manualControl:
                    colorid[3] = 3
                setButtonImage(buttonPanel,colorid)
            elif buttonPanel.states[0] > 1:
