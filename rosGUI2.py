...
# Positions and dimensions of the static button display used for specific GUI controls
BUTTONINITIALPOS = (625,20)
BUTTONDISTRIB = (1,4)
BUTTONOFFSET = (0,50)

# Set of colors that a button can use given a "button color state"
COLORBUTTONS = [[148.0,148.0,148.0],[0.0,169.0,0.0],[184.0,15.0,10.0],[0.0,109.0,180.0]]  # GRAY // GREEN // RED // BLUE


# Possible "button color states" used for certain events on GUI
BUTCOLORSTATEINI = [0,2,2,0]    # GUI just init or in normal mode without button inputs
BUTCOLORSTATEZOOM = [2,0,0,2]   # GUI in zoomed mode without button inputs

BUTCOLORSTATEACKNOWLEDGE = [1,2,2,0]
BUTCOLORSTATEIMAGESEL = [2,1,0,2]
BUTCOLORSTATESWAPREADY = [2,1,1,2]
BUTCOLORSTATEMANUAL = [0,2,2,3]

# Button text display below them
BUTTONTEXT = [['Acknowledge'],['Save'],['Swap'],['ManualCtrl']]
...
ACKNOWLEDGEKEY = pygame.K_a
SWAPKEY = pygame.K_c
MANUALKEY = pygame.K_m

# Orders to send when the corresponding screen matches (Used as well below its display)
...
class ScreenDisplay():
    ...
    # raw argument disables expected transformation to do on opencv BGR flipped stored images taken by webcam
    # (No checks are done on screenDim or image argument format)
    def update(self,image,screenDim,raw=False):
...
class ScreenDisplayPanel():
    ...
    def update(self,image,screenDim,screenNum=0,raw=False):
        if (min(self.pos) >= 0) and (self.pos[0] + self.dim[0] <= screenDim[0]) and (self.pos[1] + self.dim[1] <= screenDim[1]):
            screenNum = max(0, screenNum)
            screenNum = min(len(self.displays)-1, screenNum)
            self.displays[screenNum].update(image,screenDim,raw=raw)
    ...
    def updatepressed(self,mousepos,statevalue=1,erase=True):
        changed = False
    ...
    # (If screenNum is negative, all values would be erased unless disabled by erase parameter)
    def updatestates(self,screenNum=-1,statevalue=1,erase=True):
        if erase:
...
def setButtonImage(buttonPanel,buttoncolorid):
    for i in range(len(buttonPanel.displays)):
        buttonPanel.update(np.array([[COLORBUTTONS[buttoncolorid[i]]]]),SCREENSIZE,i,raw=True)
        buttonPanel.updatestates(i,buttoncolorid[i],erase=False)
...
class gestureGUI(Node):
    ...
    def __init__(self,displaypath,savepath,imgformat,cameraport,saveframes,usefinger,debug,nomediapipe,recognizer):
        ...
        # Create buttons tied to direct GUI actions
        self.buttonPanel = ScreenDisplayPanel(BUTTONINITIALPOS,DISPLAYINITIALDIM,BUTTONOFFSET,BUTTONDISTRIB,[],text=BUTTONTEXT)
        setButtonImage(self.buttonPanel,BUTCOLORSTATEINI)
    ...
    def handle_events(self,event,image,success):
        ...
        # All mouse events
            ...
            if self.camara.dim[0] == CAMARAZOOMEDDIM[0] and self.camara.dim[1] == CAMARAZOOMEDDIM[1]:
                if self.camara.pressed(pos):
                ...
                elif self.camaraPanel.updatepressed(pos):
                    for j in range(len(self.camaraPanel.states)):
                        if self.camaraPanel.states[j] > 0:
                            self.id = j
                    if pygame.mouse.get_pressed()[0]:
                        self.rightmouseim = self.id
                        self.camaraPanel.updatestates(self.leftmouseim,3,erase=False)
                    else:
                        self.leftmouseim = self.id
                        self.camaraPanel.updatestates(self.leftmouseim,3,erase=False)
                        self.camaraPanel.updatestates(self.rightmouseim,1,erase=False)

                    if self.rightmouseim > 0 and self.leftmouseim > 0 and self.buttonPanel.states[2] < 1:
                        setButtonImage(self.buttonPanel,BUTCOLORSTATESWAPREADY)
                    elif self.buttonPanel.states[1] < 1 or (self.buttonPanel.states[2] > 0 and self.rightmouseim == self.leftmouseim):
                        setButtonImage(self.buttonPanel,BUTCOLORSTATEIMAGESEL)


                elif (len(self.buttonPanel.states) > 3) and self.buttonPanel.updatepressed(pos,-1,erase=False):
                    if self.buttonPanel.states[1] < 0:
                        self.buttonPanel.states[1] = 1
                    ...
                    elif self.buttonPanel.states[2] < 0:
                        self.buttonPanel.states[2] = 1
                        if sum(self.camaraPanel.states) > 3:
                            self.camaraPanel.text[self.leftmouseim][0], self.camaraPanel.text[self.rightmouseim][0] = self.camaraPanel.text[self.rightmouseim][0], self.camaraPanel.text[self.leftmouseim][0]
                            tempstorage = self.camaraPanel.storage[self.rightmouseim]
                            self.camaraPanel.update(self.camaraPanel.storage[self.leftmouseim],SCREENSIZE,self.rightmouseim)
                            self.camaraPanel.update(tempstorage,SCREENSIZE,self.leftmouseim)
            ...
            # Camera on normal mode (Ready to pass to zoomed mode once live camera pressed)
                ...
                if self.camara.pressed(pos):
                    ...
                    setButtonImage(self.buttonPanel,BUTCOLORSTATEZOOM)
                    self.manualControl = False

                elif self.buttonPanel.states[0] > 0 and self.buttonPanel.updatepressed(pos,-1,erase=False):
                    if self.buttonPanel.states[0] < 0:
                        self.buttonPanel.states[0] = 2
                    elif self.buttonPanel.states[3] < 0:
                        self.manualControl = not self.manualControl
                        butcolorid = copy(BUTCOLORSTATEINI)
                        if self.manualControl:
                            butcolorid[3] = 3
                        
                        butcolorid[0] = self.buttonPanel.states[0]
                        setButtonImage(self.buttonPanel,butcolorid)


                elif self.buttonPanel.updatepressed(pos,-1,erase=False) and self.buttonPanel.states[3] < 0:
                    self.manualControl = not self.manualControl
                    butcolorid = copy(BUTCOLORSTATEINI)
                    if self.manualControl:
                        butcolorid[3] = 3
                    ...
                    self.camaraPanel.updatepressed(pos,2)
        ...
        # All keybinds events
            ...
            # Events only available on zoomed camera mode
                ...
                # Update a screen in the display with current frame of the camera
            ...
            elif key[ACKNOWLEDGEKEY] and self.buttonPanel.states[0] > 0:
                self.buttonPanel.states[0] = 2
            ...
            if self.manualControl:
                for i in range(len(DISPLAYSELECTKEYS)):
                    if key[DISPLAYSELECTKEYS[i]]:
                        self.camaraPanel.updatestates(i,2)
                        break
    ...
    def draw_GUI(self,image):
        ...
        self.buttonPanel.draw(self.screen)
    ...
    def detect_gesture_action(self,image,timer):
        ...
        # Enact the detection once timer runs out (and not on debug mode)
        elif self.camara.dim[0] == CAMARAINITIALDIM[0] and self.camara.dim[1] == CAMARAINITIALDIM[1]:

            if not self.manualControl and not self.nomediapipe:
                if self.usefinger:
            ...
            if self.buttonPanel.states[0] < 1:
                colorid = BUTCOLORSTATEACKNOWLEDGE
                if self.manualControl:
                    colorid[3] = 3
                setButtonImage(self.buttonPanel,colorid)
            elif self.buttonPanel.states[0] > 1:
                colorid = BUTCOLORSTATEINI
                if self.manualControl:
                    colorid[3] = 3
                setButtonImage(self.buttonPanel,colorid)

                for j in range(len(self.camaraPanel.displays)):
                    if self.camaraPanel.states[j] == 2:
                        gest_detected = j+1
                        #self.get_logger().info(f'Gesture matched with action: {self.camaraPanel.text[j][1]}')
                        break
    ...
    def run(self, gest_pub_topic):
        ...
        try:
            ...
            while rclpy.ok() and running:
                ...
                gest_detected,timer = self.detect_gesture_action(image,timer)
